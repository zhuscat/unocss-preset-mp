...
function getSizeValue(minmax: string, hw: string, theme: Theme, prop: string) {
  ...
  return h.cssvar.global.auto.fraction.rem(prop)
}
...
export const sizes: Rule<Theme>[] = [
  ...
  // addition rules: safe-area
  [
    /^(?:size-)?(min-|max-)?(h)-screen-safe$/,
    ([, m, h]) => ({
      [getPropName(m, h)]:
...
function getAspectRatio(prop: string) {
  if (/^\d+to\d+$/.test(prop)) return prop.replace('to', '/')
  ...
  return h.cssvar.global.auto.number(prop)
}
